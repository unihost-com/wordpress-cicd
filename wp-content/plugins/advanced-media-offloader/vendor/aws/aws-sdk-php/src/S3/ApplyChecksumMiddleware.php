...
use WPFitter\Aws\MetricsBuilder;
...
class ApplyChecksumMiddleware
{
    ...
    public function __invoke(CommandInterface $command, RequestInterface $request)
    {
        ...
        if (!empty($checksumMemberName) && !empty($requestedAlgorithm)) {
            ...
            $command->getMetricsBuilder()->identifyMetricByValueAndAppend('request_checksum', $requestedAlgorithm);
        ...
        if (!empty($checksumInfo)) {
            ...
            if (!empty($checksumRequired) || \in_array($name, self::$sha256AndMd5) && $addContentMD5) {
                ...
                if ($this->isS3Express($command)) {
                    ...
                    $command->getMetricsBuilder()->append(MetricsBuilder::FLEXIBLE_CHECKSUMS_REQ_CRC32);
